...
void transform(BoxCollider* collider) {
	...
	Matrix33 dest;
	multiplyMatrix33Matrix33(&yRot, &xRot, &dest);
	multiplyMatrix33Matrix33(&zRot, &dest, &dest);
	for (int i = 0; i < 8; i++) {
		multiplyVector3Matrix33(&dest, &baseVerts[i], &collider->next[i]);
	}
}
